...
**AirTag（第2世代）はどのように検出されますか？**
第2世代のAirTagは、単純な観察だけでは他のApple「探す」デバイスと区別できません。AirTagとして特定するには、AirGuardがトラッカーに接続する必要があります。自動検出を行うには、設定の**トラッカータイプの自動検出**がオンになっており、**デバイスタイプを除外**設定で**Apple FindMyデバイス**が無効になっていないことを確認してください。自動検出が無効になっている場合でも、第2世代のAirTagがあなたを追跡しているときはAirGuardが通知しますが、その場合は「Apple FindMyデバイス」として表示されます。
